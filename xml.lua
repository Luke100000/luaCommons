---@diagnostic disable: no-unknown, param-type-mismatch, cast-local-type
--[=====================================================================[
v0.8 Copyright © 2013-2018 Gavin Kistner <!@phrogz.net>; MIT Licensed
See http://github.com/Phrogz/SLAXML for details.
--]=====================================================================]
-- Merged into single file and type by jonas.pech@aon.at
        ...
        if first then
            ...
            if first then
                ...
                for i = #nsStack, 1, -1 do
                    if nsStack[i]['!'] then
                        currentElement[2] = nsStack[i]['!']; break
                    end
                end
            end
...
---@class SLAXMLtoDOMOpts
---@field simple boolean?
---@field indent number | string | nil
---@field omit string[]?
---@field sort boolean?
---@field stripWhitespace boolean?

---@alias SLAXMLKid SLAXMLElement | SLAXMLAttribute | SLAXMLText | SLAXMLComment | SLAXMLPI

---@class SLAXMLDocument
---@field type string
---@field name string
---@field kids SLAXMLKid[]
---@field root SLAXMLElement
...
---@field attr { [string] : string, [integer] : SLAXMLAttribute }
---@field kids SLAXMLKid[]
---@field el SLAXMLElement[] | nil
---@field parent SLAXMLElement | nil

---@class SLAXMLAttribute
---@field type string
---@field name string
---@field value string
---@field nsURI string?
...
---Convert an XML string to a DOM-like table.
---@param xml string
---@param opts SLAXMLtoDOMOpts?
---@return SLAXMLDocument
function SLAXML:dom(xml, opts)
    ...
    local builder = SLAXML:parser {
        startElement = function(name, nsURI, nsPrefix)
            ...
            if current == doc then
                if doc.root then
                    error(("Encountered element '%s' when the document already has a root '%s' element")
                        :format(name, doc.root.name))
                end
        ...
        attribute = function(name, value, nsURI, nsPrefix)
            if not current or current.type ~= "element" then
                error(("Encountered an attribute %s=%s but I wasn't inside an element")
                    :format(name, value))
            end
            local attr = {
                type = 'attribute',
        ...
        closeElement = function(name)
            if current.name ~= name or current.type ~= "element" then
                error(("Received a close element notification for '%s' but was inside a '%s' %s")
                    :format(name, current.name, current.type))
            end
            pop(stack)
            current = stack[#stack]
        end,
        text = function(value, cdata)
            ...
            if current.type == 'document' and not value:find('^%s+$') then
                error(("Document has non-whitespace text at root: '%s'")
                    :format(value:gsub('[\r\n\t]', { ['\r'] = '\\r', ['\n'] = '\\n', ['\t'] = '\\t' })))
            end
            push(current.kids,
                {
...
---@class SLAXMLtoXMLOpts
---@field indent number | string
---@field omit string[]
---@field sort boolean
---@field simple boolean
---@field stripWhitespace boolean

---Convert a table to an XML string.
---@param n table<string, any>
---@param opts SLAXMLtoXMLOpts?
---@return string
